...
pl = ggplot(diamonds, aes("color", "price", fill = "cut")) + \
...
# draw lines and outliers with darker shades of `fill` given as a float `colour`:
pl = ggplot(diamonds, aes("color", "price")) + \
          geom_boxplot(aes(fill='cut', width = 0.6, spacing=0.02,
                           colour=0.7, notch=True, notchwidth=0.0) ) + scale_y_log()
print(pl)
